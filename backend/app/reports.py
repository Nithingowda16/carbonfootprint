...
from app.models import Achievement, CarbonRecord, Goal, User, db
...
def download_pdf_report():
    ...
    story.append(
        Paragraph(
            f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", body_style
        )
    )
    story.append(Paragraph(f"User: {user.username} ({user.email})", body_style))
    story.append(
        Paragraph(
            f"Eco Level: {user.level} | Total Badges: {len(achievements)}", body_style
        )
    )
    ...
    data = [
        [
            Paragraph("Category", bold_body_style),
            Paragraph("Total Emissions (kg CO2)", bold_body_style),
        ],
        [
            Paragraph("Transportation", body_style),
            Paragraph(f"{t_sum:.2f}", body_style),
        ],
        ...
        [
            Paragraph("Waste Management", body_style),
            Paragraph(f"{w_sum:.2f}", body_style),
        ],
        [
            Paragraph("Grand Total", bold_body_style),
            Paragraph(f"{total_co2:.2f}", bold_body_style),
        ],
        [
            Paragraph("Daily Average", bold_body_style),
            Paragraph(f"{avg_co2:.2f}", bold_body_style),
        ],
    ]
    ...
    score_data = [
        [
            ...
            Paragraph("Rating", bold_body_style),
        ],
        [
            Paragraph("Carbon Score", body_style),
            Paragraph(str(carbon_score), body_style),
            Paragraph(
                "Excellent"
                if carbon_score > 80
                else ("Good" if carbon_score > 50 else "Needs Improvement"),
                body_style,
            ),
        ],
        [
            Paragraph("Green Points Balance", body_style),
            Paragraph(str(user.green_points), body_style),
    ...
    if goals:
        goals_data = [
            [
                Paragraph("Category", bold_body_style),
                Paragraph("Target %", bold_body_style),
        ...
        story.append(
            Paragraph(
                "You do not have any active goals set yet. Set goals on your EcoTrack AI dashboard to start saving carbon!",
                body_style,
            )
        )
    ...
    story.append(
        Paragraph(
            "EcoTrack AI | Keep tracking and make every day greener!", bold_body_style
        )
    )
