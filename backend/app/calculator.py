...
def calculate_co2(details):
    ...
    transport_co2 = (
        (
            car_km
            * TRANSPORT_FACTORS.get(
                f"car_{trans_type}", TRANSPORT_FACTORS["car_petrol"]
            )
        )
    ...
    waste_co2 = (waste_kg * WASTE_FACTORS["waste_kg"]) + WASTE_FACTORS.get(
        f"plastic_{plastic_level}", WASTE_FACTORS["plastic_medium"]
    )
    if recycles:
...
def get_history():
    ...
    records = (
        ...
        .order_by(CarbonRecord.date.desc())
        .limit(30)
        .all()
    )
        ...
        if not records:
            ...
            if avg_current <= goal.target_co2:
                ...
                pct = max(
                    0,
                    int(
                        100 - ((avg_current - goal.target_co2) / goal.target_co2 * 100)
                    ),
                )
...
def log_action():
    ...
    action_count = (
        Action.query.filter_by(user_id=user_id, status="completed").count() + 1
    )
    if action_count == 1:
        db.session.add(
            Achievement(
                user_id=user_id, badge_key="first_action", badge_name="Green Novice"
            )
        )
    elif action_count == 10:
        db.session.add(
            Achievement(
                user_id=user_id, badge_key="eco_warrior", badge_name="Eco Warrior"
            )
        )
...
def get_actions():
    user_id = int(get_jwt_identity())
    actions = (
        Action.query.filter_by(user_id=user_id)
...
def join_challenge(challenge_id):
    ...
    if existing:
        return jsonify(
            {"message": "Already joined", "user_challenge": existing.to_dict()}
        ), 200

    uc = UserChallenge(
        user_id=user_id, challenge_id=challenge_id, progress=0.0, status="joined"
    )
    db.session.add(uc)
    db.session.commit()
...
def log_challenge_progress(challenge_id):
    ...
    if not uc:
        return jsonify(
            {"error": "Challenge assignment not found. Join the challenge first."}
        ), 404
