...
def login():
    ...
    if last_active_date == today - timedelta(days=1):
        ...
        if user.streak == 7:
            badge = Achievement(user_id=user.id, badge_key="streak_7", badge_name="Weekly Streak Master")
            db.session.add(badge)
            user.xp += 100
...
def forgot_password():
    ...
    if not user:
        return jsonify({'error': 'If that email is registered, we have sent instructions'}), 200
