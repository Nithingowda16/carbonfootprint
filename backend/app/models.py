...
class User(db.Model):
    ...
    actions = db.relationship(
        "Action", backref="user", lazy=True, cascade="all, delete-orphan"
    )
    user_challenges = db.relationship(
        "UserChallenge", backref="user", lazy=True, cascade="all, delete-orphan"
    )
...
class Goal(db.Model):
    ...
    category = db.Column(
        db.String(50), default="overall", nullable=False
    )  # overall, transport, home, food, waste
...
class Action(db.Model):
    ...
    )  # e.g. walk_short, turn_off_appliances
    action_type = db.Column(
        db.String(50), nullable=False
    )  # transport, home, food, waste
    carbon_reduction = db.Column(
        db.Float, nullable=False
    )  # kg CO2 reduced per completion
...
class Challenge(db.Model):
    ...
    target_value = db.Column(
        db.Float, nullable=False
    )  # e.g. save 10kg CO2, or walk 5 times
    challenge_type = db.Column(
        db.String(50), nullable=False
    )  # transport, home, food, waste, general
    start_date = db.Column(db.Date, nullable=False)
    end_date = db.Column(db.Date, nullable=False)
...
class UserChallenge(db.Model):
    ...
    status = db.Column(
        db.String(20), default="joined", nullable=False
    )  # joined, completed
    joined_at = db.Column(
        db.DateTime, default=lambda: datetime.now(timezone.utc), nullable=False
    )
...
class Achievement(db.Model):
    ...
    badge_key = db.Column(
        db.String(50), nullable=False
    )  # first_step, eco_warrior, etc.
    badge_name = db.Column(
        db.String(100), nullable=False
    )  # "First Green Step", "Eco Warrior"
